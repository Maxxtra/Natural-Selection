...
class organism:
    ...
    def __init__(self, window, x, y, size, speed, organismType):
        ...
        if self.type == 'omnivore':
            ...
            self.organismGraphic.setFill('yellow')
    ...
    def moveRandom(self):
        ...
        '''
        self.velocity = (
        random.uniform(-self.speed, self.speed) * speedFactor, random.uniform(-self.speed, self.speed) * speedFactor)
    ...
    def checkCollision(self, collidingOrganism):
        ...
        if (self.pos[0] - self.size) <= self.collidingOrganismPos[0] <= (self.pos[0] + self.size):
            if (self.pos[1] - self.size) <= self.collidingOrganismPos[1] <= (self.pos[1] + self.size):
                return True
        elif (self.collidingOrganismPos[0] - self.collidingOrganismsize) <= self.pos[0] <= (
                self.collidingOrganismPos[0] + self.collidingOrganismsize):
            if (self.collidingOrganismPos[1] - self.collidingOrganismsize) <= self.pos[1] <= (
                    self.collidingOrganismPos[1] + self.collidingOrganismsize):
                return True
        return False
...
class naturalSelectionSim:
    ...
    def displayInformation(self):
        ...
        window.
        '''
        self.informativeText_1 = Text(Point(0, -105), 'Click anywhere inside to add a new organism.')
        self.informativeText_1.setSize(20)
        self.informativeText_1.draw(self.window)

        self.plantExample = Circle(Point(-90, -115), 3)
        self.plantExample.setFill(color_rgb(10, 117, 31))
        self.plantExample.draw(self.window)
        self.plantTitle = Text(Point(-75, -115), '    =  Plant')
        self.plantTitle.setSize(20)
        self.plantTitle.draw(self.window)

        self.herbivoreExample = Circle(Point(-90, -125), 3)
        self.herbivoreExample.setFill('yellow')
        self.herbivoreExample.draw(self.window)
        self.herbivoreTitle = Text(Point(-69, -125), '      =  Herbivore')
        self.herbivoreTitle.setSize(20)
        self.herbivoreTitle.draw(self.window)

        self.carnivoreExample = Circle(Point(-90, -135), 3)
        self.carnivoreExample.setFill('red')
        self.carnivoreExample.draw(self.window)
        self.carnivoreTitle = Text(Point(-69, -135), '      =  Carnivore')
        self.carnivoreTitle.setSize(20)
        self.carnivoreTitle.draw(self.window)

        self.omnivoreExample = Circle(Point(-90, -145), 3)
        self.omnivoreExample.setFill('blue')
        self.omnivoreExample.draw(self.window)
        self.omnivoreTitle = Text(Point(-69, -145), '      =  Omnivore')
    ...
    def spawnOrganismFromInput(self):
        ...
        if self.mousePos != None:
            if abs(self.mousePos.getX()) < windowSize and abs(self.mousePos.getY()) < windowSize:
                self.organisms.append(self.randomOrganism(random.choice(('omnivore', 'carnivore', 'herbivore', 'plant')),
                                                          (self.mousePos.getX(), self.mousePos.getY())))
